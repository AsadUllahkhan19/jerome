...
function getTheme(
  theme: Themes,
  overrides?: Omit<RangeSliderOptions, "leftInputProps" | "rightInputProps">
) {
...
export function RangeSlider(props: RangeSliderProps): JSX.Element {
  let { style, min, max, step, options, displayValues, ...wrapperProps } =
    props;
  let { fromDate, setFromDate, toDate, setToDate } = displayValues;
  let { theme = "default", leftInputProps, rightInputProps } = { ...options };

  /** SANITY CHECKS */
  if (min > max)
    throw new Error(
      "next-range-slider: min must be less than max in component props."
    );
  ...
  let _style = {
    ...
    ...minMaxTags(
      min,
      max,
      Number(_leftInputProps.value ?? 0),
      Number(_rightInputProps.value ?? 0)
    ),
  } as CSSProperties;
  const leftPercentage = Number(_leftInputProps.value) / 4;
  const rightPercentage = Number(_rightInputProps.value) / 4;
  // var currentDate = new Date(2024, 5, 2);
  var currentDate = new Date();
  var currentDayOfYear = Math.ceil(
    // @ts-ignore
    (currentDate - new Date(currentDate.getFullYear(), 0, 1)) /
      (1000 * 60 * 60 * 24)
  ) as number;
  function CalculateFromDate(leftInputPropsValue: number) {
    const DisplayYear =
      currentDayOfYear + leftInputPropsValue > 365
    ...
    var selectedDate = new Date(
      DisplayYear,
      0,
      currentDayOfYear + leftInputPropsValue
    );
    var monthNames = [
      "Jan",
      "Feb",
  ...
  function CalculateToDate(leftInputPropsValue: number) {
    const DisplayYear =
      currentDayOfYear + leftInputPropsValue > 365
        ? currentDate.getFullYear() + 1
        : currentDate.getFullYear();
  ...
  useEffect(() => {
    CalculateFromDate(Number(_leftInputProps.value));
    CalculateToDate(Number(_rightInputProps.value));
  }, [_leftInputProps.value, _rightInputProps.value]);
  return (
    <div className="wrap" role="group" style={_style} {...wrapperProps}>
      <div
        style={{ left: `${leftPercentage}%` }}
        className={`bg-white mt-6 text-xs border px-2 py-2 text-center  border-black top-0 absolute`}
      >
        {fromDate}
      </div>
      <div
        style={{ left: `${rightPercentage - 26}%` }}
        className={`bg-white mt-6 text-xs border px-2 py-2 text-center  border-black -top-[70px] absolute`}
      >
        {toDate}
      </div>
      <input
